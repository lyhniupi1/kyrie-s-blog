...
from myblog.forms import SettingForm, PostForm, CategoryForm, LinkForm, SkillForm
from myblog.models import Post, Category, Comment, Link, Skill
...
#Skill
@admin_bp.route("/new_skill", methods=["GET","POST"])
def new_skill():
    form = SkillForm()
    if form.validate_on_submit():
        link = Skill(name=form.name.data)
        db.session.add(link)
        db.session.commit()
        return redirect(url_for("blog.index"))
    return render_template("/admin/new_skill.html", form=form)


@admin_bp.route("/manage_skill", methods=["GET","POST"])
def manage_skill():
    skills = Skill.query.all()
    return render_template("admin/manage_skill.html", skills=skills)
...
@admin_bp.route("/edit_skill/<int:skill_id>", methods=["GET","POST"])
