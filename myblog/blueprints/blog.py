...
@blog_bp.route("/change_theme/<string:theme_name>")
...
@blog_bp.route("/post/reply/<comment_id>")
def reply_comment(comment_id):
    comment = Comment.query.get_or_404(comment_id)
    if not comment.post.can_comment:
        flash("Comment is disabled", "warning")
        return redirect(url_for("blog.show_post", post_id=comment.post.id))
    return redirect(url_for("blog.show_post", post_id=comment.post.id, reply=comment_id, author=comment.author) + "#comment-form")